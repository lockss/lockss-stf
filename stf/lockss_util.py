...
class Configuration(dict):
    ...
    def daemonItems(self):
        ...
        return Configuration((key, value) for key, value in self.iteritems()
    ...
    def load(self, filename):
        ...
        def error(line_number):
            raise LockssError('Invalid expression on line %i of %s' %
...
class Multipart_Form_Request(urllib2.Request):
    ...
    def __init__(self,
                 ...
                 files=None,
                 headers=None,
                 origin_request_host=None,
                 unverifiable=False):
        urllib2.Request.__init__(self, url, data, {} if headers is None else
                                 headers, origin_request_host, unverifiable)
        self.files = files
    ...
    def http_request(self, request):
        def multipart_encode(data, files, boundary):
            ...
            for key, filename in files.iteritems():
                ...
                form.append('Content-Type: ' + (mimetypes.guess_type(
        ...
        if isinstance(request, Multipart_Form_Request) and request.has_files():
            ...
            request.add_data(
                multipart_encode(request.get_data(), request.get_files(),
                                 boundary))
...
def HTTP_request(opener, url, data=None, files=None):
    ...
    request = Multipart_Form_Request(url, encoded_data, files, None
                                     if data and 'output' in data else
                                     {'X-Lockss-Result': 'Please'})
    session_lock = session_locks.setdefault(request.get_host(),
                                            threading.Lock())
    session_lock.acquire(
    )  # Server will dispense multiple initial session cookies if addressed concurrently
    try:
        log.debug2("Sending HTTP request for %s: %s, %s" %
                   (request.get_full_url(), encoded_data, files))
        resource = opener.open(request)
        log.debug2("Received HTTP reply  for %s: %s, %s" %
                   (request.get_full_url(), encoded_data, files))
    finally:
